...
__conda_setup="$('/home/phucvd/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
...
# uv / uvx zsh completion. Sourced from static files pre-generated by
# scripts/programs/uv.sh so startup stays fast (no `uv` subprocess per shell).
# antigen has already run compinit above, so compdef is available here.
[[ -f ~/.config/uv/uv.zsh ]] && source ~/.config/uv/uv.zsh
[[ -f ~/.config/uv/uvx.zsh ]] && source ~/.config/uv/uvx.zsh
