...
-- Scroll horizontally one column at a time rather than half a screen, which
-- only matters once 'wrap' is off (see the <leader>tw toggle below).
vim.o.sidescroll = 1
...
-- Some of the markdown tables in this repo are 175-270 columns wide. With 'wrap'
-- on, each row spills over several screen lines and render-markdown's cell
-- padding fills the overflow with blanks, so the closing `|` lands on a line of
-- its own. Turning 'wrap' off puts every row back on one line, aligned, and you
-- scroll sideways instead. Handy for any long-line file, not just markdown.
